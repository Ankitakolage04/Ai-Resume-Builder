...
import { FaBrain, FaTrash, FaPlusCircle, FaRocket, FaMagic } from "react-icons/fa";
...
const FormField = ({ label, name, register, type = "text", placeholder = "" }) => (
...
const DynamicSection = ({ label, name, control, register, keys }) => {
  ...
  return (
    <div className="card bg-base-100 shadow-lg border border-base-200 mb-8 overflow-hidden">
      <div className="bg-base-200/50 p-4 border-b border-base-200 flex justify-between items-center">
        ...
        <button
          type="button"
          onClick={() => append(keys.reduce((acc, key) => ({ ...acc, [key]: "" }), {}))}
      ...
      <div className="card-body p-4">
                ...
                {keys.map((key) => (
                  <div key={key} className={key === 'description' || key === 'responsibility' || key === 'summary' || key === 'extraInformation' ? 'col-span-full' : ''}>
                    {key === 'description' || key === 'responsibility' || key === 'summary' || key === 'extraInformation' ? (
                      ...
                      <FormField
                        key={key}
                        label={key.charAt(0).toUpperCase() + key.split(/(?=[A-Z])/).join(" ")}
                        name={`${name}.${index}.${key}`}
                        register={register}
                      />
                    )}
        ...
        {fields.length === 0 && <p className="text-center text-gray-400 py-4 italic">No {label.toLowerCase()} added yet. Click &quot;Add Item&quot; to start.</p>}
      </div>
    </div>
  );
};
          ...
          <div className="container mx-auto max-w-6xl">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 animate-fade-in pb-24">
              ...
              <div className="card bg-base-100 shadow-xl border-t-4 border-primary">
                <div className="card-body">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <FormField label="Full Name" name="personalInformation.fullName" register={register} />
                    <FormField label="Email" name="personalInformation.email" register={register} type="email" />
                    <FormField label="Phone" name="personalInformation.phoneNumber" register={register} />
                    <FormField label="Location" name="personalInformation.location" register={register} />
                    <FormField label="Portfolio URL" name="personalInformation.portfolio" register={register} type="url" />
                    <FormField label="LinkedIn URL" name="personalInformation.linkedIn" register={register} type="url" />
                    <FormField label="GitHub URL" name="personalInformation.gitHub" register={register} type="url" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <DynamicSection label="Work Experience" name="experience" control={control} register={register} keys={["jobTitle", "company", "location", "duration", "responsibility"]} />
                <DynamicSection label="Projects" name="projects" control={control} register={register} keys={["title", "description", "technologiesUsed", "githubLink"]} />
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <DynamicSection label="Education" name="education" control={control} register={register} keys={["degree", "university", "location", "graduationYear"]} />
                <DynamicSection label="Certifications" name="certifications" control={control} register={register} keys={["title", "issuingOrganization", "year"]} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <DynamicSection label="Skills" name="skills" control={control} register={register} keys={["title", "level"]} />
                <DynamicSection label="Languages" name="languages" control={control} register={register} keys={["name"]} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <DynamicSection label="Interests" name="interests" control={control} register={register} keys={["name"]} />
                <DynamicSection label="References" name="references" control={control} register={register} keys={["name", "designation", "company", "contact"]} />
              </div>

              {/* Achievements Section - NEW */}
              <DynamicSection label="Achievements" name="achievements" control={control} register={register} keys={["title", "year", "extraInformation"]} />
